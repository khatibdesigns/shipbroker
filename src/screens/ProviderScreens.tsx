...
export function FreightForwarder() {
  ...
  const items: { k: string; label: string; sub?: string }[] = [
    { k: 'fcl', label: t('FCL', 'حمولة حاوية كاملة'), sub: t('Full Container Load', '') },
    { k: 'lcl', label: t('LCL', 'أقل من حاوية'), sub: t('Less than Container', '') },
    ...
    { k: 'reefer', label: t('Refrigerated', 'مبرّد'), sub: t('Reefer transport', '') },
  ...
  return (
    <Screen>
      ...
      <FooterBar left={t('Cancel', 'إلغاء')} right={t('Next', 'التالي')} onLeft={nav.pop} onRight={() => { nav.popToRoot(); nav.switchTab('home'); }} />
    </Screen>
  );
}
...
export function Trucking() {
  ...
  const toggle = (k: string) => setSel((s) => (s.includes(k) ? s.filter((x) => x !== k) : [...s, k]));
  const rows: { k: string; label: string; sub: string; icon: IconName }[] = [
  ...
  return (
    <Screen>
      ...
      <Body contentStyle={{ paddingTop: 12 }}>
        ...
        <Link label={t('Show more', 'عرض المزيد')} brand style={{ marginTop: 14 }} />
      </Body>
      <FooterBar left={t('Back', 'رجوع')} right={t('Next', 'التالي')} onLeft={nav.pop} onRight={() => { nav.popToRoot(); nav.switchTab('home'); }} />
    </Screen>
  );
}
  ...
  return (
    <Screen>
      ...
      <Body contentStyle={{ paddingTop: 14 }}>
        ...
        <Field label={t('Captain license type', 'نوع رخصة الربّان')} ph={t('Select license type', 'اختر نوع الرخصة')} select />
        <Txt size={12} weight="semibold" color={colors.textSecondary} style={{ marginTop: -10, marginBottom: 16, marginHorizontal: 2, lineHeight: 18 }}>
          {t('Coastal/Nearshore · Offshore · Yachtmaster · Master 200 GT · Master 500 GT', 'ساحلي · بحري · ماستر يخوت · ماستر 200 · ماستر 500')}
        </Txt>
        <Row gap={12}>
          <View style={{ flex: 1 }}>
            <Field label={t('Max boat length (m)', 'أقصى طول للقارب (م)')} ph="e.g. 24" />
          </View>
          <View style={{ flex: 1 }}>
            <Field label={t('Max GT (optional)', 'أقصى حمولة (اختياري)')} ph="e.g. 100" />
          </View>
        </Row>
      </Body>
      <FooterBar left={t('Cancel', 'إلغاء')} right={t('Save & continue', 'حفظ ومتابعة')} onLeft={nav.pop} onRight={() => { nav.popToRoot(); nav.switchTab('home'); }} />
    </Screen>
  );
}
